...
class TouchImageView @JvmOverloads constructor(
    ...
    private fun configureImageView(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        val attributes =
            context.theme.obtainStyledAttributes(attrs, R.styleable.TouchImageView, defStyleAttr, 0)
        try {
            if (!isInEditMode) {
    ...
    private fun fitImageToView() {
        ...
        if (!isZoomed && !imageRenderedAtLeastOnce) {
            ...
            if (isRotateImageToFitScreen && orientationMismatch(drawable)) {
                ...
                matrix!!.postTranslate(drawableWidth.toFloat(), 0f)
    ...
    fun setZoomAnimated(scale: Float, focusX: Float, focusY: Float, zoomTime: Int) {
